...
CONFIG = {
    ...
    "movementNoise": 0.01,  # Small random noise added to movement (0.0 = no noise)
    ...
    "cruiseSpeed": 2,
    "burstSpeed": 4.0,
    "maxAngularVelocitySlow": 0.12,  # Increased from 0.08 for faster turning when slow
    "maxAngularVelocityFast": 0.05,  # Increased from 0.03 for better maneuverability at speed
    ...
    "formationQualityThreshold": 0.55,  # How well predators must be positioned (0-1)
    "K_FORMATION": 0.25,  # Weight for moving toward assigned formation position (increased from 0.15)
    "K_SEPARATION": 0.03,  # Weight for maintaining distance from other predators (reduced from 0.05)
    "minPredatorSeparation": 80,  # Minimum distance between predators (reduced from 100)
    "compressionForce": 0.15,  # Force pushing toward prey to compress the group (increased from 0.1)
...
class Predator(Agent):
    ...
    def formation_acceleration(self, formation_pos, prey_centroid):
        ...
        # Boost speed when far from formation position
        speed_boost = 1.0
        if dist_to_formation > 100:
            speed_boost = 1.5  # Move 50% faster when positioning
            # Temporarily increase max speed for faster positioning
            if self.max_speed < CONFIG['burstSpeed'] * 0.75:
                self.max_speed = CONFIG['burstSpeed'] * 0.75  # Use 75% of burst speed
        
        formation_force = to_formation * CONFIG['K_FORMATION'] * speed_boost
        
        # Add tangential component to circle around prey (only when close to position)
    ...
    def compression_acceleration(self, prey_centroid):
        ...
        # Apply compression force scaled by distance
        if dist > CONFIG['formationRadius']:
            compression = to_prey.normalize() * CONFIG['compressionForce']
            return compression
        elif dist > CONFIG['formationRadius'] * 0.5:
            # Stronger compression when inside formation radius
            compression = to_prey.normalize() * CONFIG['compressionForce'] * 1.5
            return compression
